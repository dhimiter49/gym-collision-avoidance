...
class Config(object):
    def __init__(self):
        ...
        ### TRAIN / PLAY / EVALUATE
        self.TRAIN_MODE = (
            True  # Enable to see the trained agent in action (for testing)
        )
        self.PLAY_MODE = (
            False  # Enable to see the trained agent in action (for testing)
        )
        self.EVALUATE_MODE = (
            False  # Enable to see the trained agent in action (for testing)
        )
        ...
        self.REWARD_COLLISION_WITH_WALL = (
            -0.25
        )  # reward given when agent collides with wall
        self.REWARD_GETTING_CLOSE = (
            -0.1
        )  # reward when agent gets close to another agent (unused?)
        self.REWARD_ENTERED_NORM_ZONE = (
            -0.05
        )  # reward when agent enters another agent's social zone
        ...
        self.GETTING_CLOSE_RANGE = (
            0.2  # meters between agents' boundaries for collision
        )
        ...
        # agent has this number times the straight-line-time to reach its goal before
        # "timing out"
        self.MAX_TIME_RATIO = 2.0
        ...
        self.MAX_NUM_OTHER_AGENTS_IN_ENVIRONMENT = (
